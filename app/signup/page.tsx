  ...
  return (
    <div className="relative w-full h-full flex flex-col overflow-hidden" style={{ background: "#000" }}>
          ...
          {[1, 2].map((s) => (
            ...
            >
              <div
                ...
                style={{ width: s <= step ? "100%" : "0%", backgroundColor: "white" }}
              />
            </div>
          ))}
        </div>
      ...
      <div className="relative z-10 flex-1 px-5 flex flex-col gap-4">
        {/* Error */}
        {error && (
          <div className="px-4 py-3 rounded-2xl text-[13px] text-red-300" style={{ backgroundColor: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.2)" }}>
            {error}
          </div>
        )}
      ...
      <div className="relative z-10 px-5 pb-10 pt-6 flex flex-col gap-3">
        <button
          ...
          onClick={() => {
            if (step === 1 && step1Ok) { setError(null); setStep(2) }
            else if (step === 2 && step2Ok) handleSignup()
          }}
